...
LOG_BASE = "/var/log/centralizacion"


def ejecutar_ssh(comando):
    try:
        resultado = subprocess.run(
            ["ssh", SSH_TARGET, comando],
            capture_output=True,
            text=True,
            timeout=10
        )
...
def get_hosts():
    return ejecutar_ssh(f"ls {LOG_BASE}")


def get_logs(host):
    return ejecutar_ssh(f"ls {LOG_BASE}/{host}")


def read_log(host, logfile, filter_type=None, search=None):
    comando = f"tail -n 200 {LOG_BASE}/{host}/{logfile}"
    lines = ejecutar_ssh(comando)
